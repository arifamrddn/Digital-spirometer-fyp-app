...
export default class Results2 extends React.Component {
  state ={
    currentUser:"",
    volume1:"",
    dateFB: [],
  ...
  render() {
    ...
    firebase.database().ref('/users/'+currentUser+'/date/'+ dateValue).once('value',(snapshot) =>{
    let dateFB = [];
...
for(let i = 0; i < this.state.dateFB.length; i++){
    let timeValue = this.state.dateFB[i];
		displayDate.push(
			<CheckBox
    ...
    something: dateValue, something2: timeValue, currentUser: this.state.currentUser
})}
          />
